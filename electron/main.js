...
function registerTableImportHandlers() {
  const {
    classifyClient,
    markSuspicious,
  } = require("./modules/classify-client");
  ...
  ipcMain.handle("table:importFeishu", async (_e, baseToken, tableId) => {
    try {
      ...
      const TARGETS = [
        ...
        {
          keys: ["公司类型", "品类", "行业", "Category", "category", "rubro"],
          field: "category",
        },
        {
          keys: ["邮箱", "联系方式", "邮箱地址", "Email", "email", "收件人"],
          field: "email",
        },
        {
          keys: ["网站", "Website", "website", "官网", "LinkedIn"],
          field: "website",
        },
        {
          keys: ["名", "FirstName", "first_name", "firstname"],
          field: "firstName",
        },
        {
          keys: ["姓", "LastName", "last_name", "lastname"],
          field: "lastName",
        },
        {
          keys: ["姓名", "联系人", "Contact", "contact"],
          field: "contactName",
        },
      ...
      for (const t of TARGETS) {
        const name = allFieldNames.find((n) =>
          t.keys.some((k) => n === k || (n && n.includes(k))),
        );
      ...
      while (true) {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          const row = rows[i];
          const colMap = {};
          (resp.data?.fields || []).forEach((name, ci) => {
            colMap[name] = ci;
          });
          const obj = {};
          for (let ti = 0; ti < TARGETS.length; ti++) {
            ...
            if (Array.isArray(val))
              clean = String(
                val[0]?.link || val[0]?.text || val[0]?.url || val[0] || "",
              );
