...
async function openDetailPanel(contactId) {
  ...
  panel.innerHTML = `
    ...
    <div class="crm-detail-body">
      ...
      <div class="crm-tab-content${_currentTab==='relations'?' active':''}" data-content="relations">${relationsTab()}</div>
    </div>`;
  ...
  panel.querySelectorAll('.crm-tab').forEach(tab => {
    tab.addEventListener('click', async () => {
      ...
      if (tab.dataset.tab === 'relations') {
        const contactId = _currentDetailId;
        if (contactId) initRelationsGraph(contactId);
      }
...
const R_STAGE = { reaching:'#ff9800', quoting:'#2196f3', trial:'#8e24aa', cooperating:'#4caf50', lost:'#b0b0b0' };
...
async function initRelationsGraph(contactId) {
  const wrap = document.getElementById('crm-rel-wrap');
  const svg = document.getElementById('crm-rel-svg');
  if (!wrap || !svg) return;

  if (_relCache && _relCache.contactId === contactId) {
    const r = _relCache;
...
function renderSimulation(svgEl, wrapEl, nodes, edges) {
  ...
  if (_resOb) { _resOb.disconnect(); _resOb = null; }
  ...
  try {
    ...
    function showRelCtx(event, nodeData, allNodes) {
      ...
      menu.querySelectorAll('[data-action="add-rel"]').forEach(el => {
        el.addEventListener('click', async () => {
          ...
          if (r.ok) { showToast('关系已添加', 'ok'); _relCache = null; initRelationsGraph(_currentDetailId); }
          else { showToast(r.error || '保存失败', 'err'); }
          menu.remove();
        });
      });
    ...
    _resOb = new ResizeObserver(() => {
      clearTimeout(wrapEl._rd);
...
function closeDetailPanel() {
  if (_sim) { _sim.stop(); _sim = null; }
  if (_resOb) { _resOb.disconnect(); _resOb = null; }
  const relSvg = document.getElementById('crm-rel-svg');
  if (relSvg) relSvg.innerHTML = '';
  _relCache = null;
